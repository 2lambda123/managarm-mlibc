...
using file_list = frg::intrusive_list<
    abstract_file,
    frg::locate_member<abstract_file, frg::default_list_hook<abstract_file>,
...
//     open (e.g. for std{in,out,err}), we defer the type determination and
//     cache the result.
...
abstract_file::abstract_file(void (*do_dispose)(abstract_file *))
  ...
  // TODO: For __fwriting to work correctly, set the __io_mode to 1 if the write
  // is write-only.
...
abstract_file::~abstract_file() {
  if (__dirty_begin != __dirty_end)
    mlibc::infoLogger()
        << "mlibc warning: File is not flushed before destruction"
        << frg::endlog;
...
// While read() can trigger a write-back, write() can never trigger a
// read-ahead(). This peculiarity is reflected in their code.
...
int abstract_file::read(char *buffer, size_t max_size, size_t *actual_size) {
  ...
  // TODO: In order to support pipe-like streams we need to write-back the
  // buffer.
  ...
  if (__offset == __valid_limit) {
    // TODO: We only have to write-back/reset if __valid_limit reaches the
    // buffer end.
...
int abstract_file::write(const char *buffer, size_t max_size,
                         size_t *actual_size) {
  __ensure(max_size);
  ...
  if (globallyDisableBuffering || _bufmode == buffer_mode::no_buffer) {
    ...
    __ensure(io_size > 0 &&
             "io_write() is expected to always write at least one byte");
    *actual_size = io_size;
    return 0;
  }
  ...
  if (!__io_mode &&
      __valid_limit) // TODO: Only check this for pipe-like streams.
    mlibc::panicLogger() << "mlibc: Cannot read-write to same pipe-like stream"
                         << frg::endlog;
  __io_mode = 1;
...
int abstract_file::_write_back() {
  ...
  if (_type == stream_type::file_like) {
    if (__io_offset != __dirty_begin) {
      ...
      off_t new_offset;
      if (int e = io_seek(off_t(__dirty_begin) - off_t(__io_offset), SEEK_CUR,
                          &new_offset);
          e)
        return e;
  ...
  while (__io_offset < __dirty_end) {
    size_t io_size;
    if (int e = io_write(__buffer_ptr + __io_offset, __dirty_end - __io_offset,
                         &io_size);
        e) {
      __status_bits |= __MLIBC_ERROR_BIT;
      return e;
...
int fd_file::determine_bufmode(buffer_mode *mode) {
  // When isatty() is not implemented, we fall back to the safest default (no
  // buffering).
  if (!mlibc::sys_isatty) {
  ...
  if (int e = mlibc::sys_isatty(_fd); !e) {
    ...
    *mode = buffer_mode::full_buffer;
    ...
    mlibc::infoLogger()
        << "mlibc: sys_isatty() failed while determining whether"
...
int fd_file::io_write(const char *buffer, size_t max_size,
                      size_t *actual_size) {
  ssize_t s;
...
int fd_file::parse_modestring(const char *mode) {
  ...
  if (*mode == 'r') {
    ...
    mlibc::infoLogger() << "Illegal fopen() mode '" << *mode << "'"
                        << frg::endlog;
  }
  mode += 1;
  ...
  while (*mode) {
    if (*mode == '+') {
      ...
      mode++; // mlibc assumes that there is no distinction between text and
              // binary.
    } else if (*mode == 'e') {
      flags |= __MLIBC_O_CLOEXEC;
      mode++;
    } else {
      mlibc::infoLogger() << "Illegal fopen() flag '" << mode << "'"
                          << frg::endlog;
      mode++;
    }
  }
...
FILE *fopen(const char *path, const char *mode) {
  ...
  return frg::construct<mlibc::fd_file>(
      getAllocator(), fd, [](mlibc::abstract_file *abstract) {
...
int fflush_unlocked(FILE *file_base) {
  if (file_base == NULL) {
    ...
    for (auto it : mlibc::global_file_list()) {
      if (int e = it->flush(); e)
        mlibc::infoLogger()
            << "mlibc warning: Failed to flush file" << frg::endlog;
    }
    return 0;
  }
  auto file = static_cast<mlibc::abstract_file *>(file_base);
